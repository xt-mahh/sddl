...
质量约束：
- **双重校验**：生成器 A 产出 → 校验器 B（不同模型）检查覆盖度（所有 THEN/ERR/SIDE 都有项、无不可验证项）
...
- **启发式 fallback 陷阱（T1 实测）**：无 LLM 时用启发式（AST + 结构化关键词）会假阴性爆炸（C1-sem 0.48）。启发式只可作初步筛选，必须人工复核；判定项需语义化（AST 提取断言 + 结构化关键词），且失败项逐个看是"真问题"还是"匹配问题"。
